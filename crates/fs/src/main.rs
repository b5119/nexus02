...
async fn main() -> Result<()> {
    ...
    match args.command {
        Command::Mount {
            ...
            let (token_val, ca_pem, maybe_client_identity) = if trusted {
                ...
                // Load this device's identity cert+key for mTLS.
                ...
                (String::new(), entry.cert_pem, Some((cert_pem, key_pem)))
            } else {
                let token =
            ...
            let client = match maybe_client_identity {
                ...
                None => grpc_client::RemoteFs::connect(remote.clone(), ca_pem, token_val).await?,
            };
