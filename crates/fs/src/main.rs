    ...
    match args.command {
        Command::Mount {
            ...
            trusted,
                ...
                let (token_val, ca_pem, maybe_client_identity, effective_device_id) = if trusted {
                ...
                // Use the device_id embedded in the cert, not the local
                // ClientConfig (the cert may have been generated by a
                // different invocation).
                let cert_device_id =
                    pairing::extract_device_id_from_cert_pem(&cert_pem)
                        .context("extracting device ID from client cert")?;

                (String::new(), entry.cert_pem, Some((cert_pem, key_pem)), cert_device_id)
            } else {
                let token =
                ...
                (token, ca_pem, None, cfg.device_id)
            };
